...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Top-level RoboTwin dataset directory (required for --format robotwin_multitask)",
    )
    parser.add_argument(
        "--robot", type=str, default="aloha-agilex", help="Robot name (for --format robotwin_multitask)"
    )
    parser.add_argument(
        "--variant", type=str, default="clean_50", help="Data variant (for --format robotwin_multitask)"
    )
    ...
    if args.format == "robotwin_multitask":
        ...
        print(f"Computing multi-task action stats: {args.dataset_dir} (robot={args.robot}, variant={args.variant})")
        if args.variant in {"train", "val"}:
            ...
            for task_idx, (task_name, data_root) in enumerate(roots):
                ...
                print(f"  [{task_idx + 1}/{len(roots)}] {task_name}: {len(files)} episodes, {task_count} timesteps")
            ...
            stats = compute_multitask_robotwin_stats(args.dataset_dir, args.robot, args.variant, tasks)
    else:
