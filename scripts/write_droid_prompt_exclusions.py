...
def main() -> None:
    ...
    if out.exists():
        try:
            ...
            if "droid_prompt_exclusions" in previous:
                previous_prompt = previous["droid_prompt_exclusions"]
                if not isinstance(previous_prompt, dict):
                    raise ValueError("droid_prompt_exclusions must be an object")
                prompt_owned = _parse_episode_indices(
                    previous_prompt["episode_indices"],
                    "droid_prompt_exclusions.episode_indices",
                )
                if not prompt_owned <= existing:
                    raise ValueError("droid_prompt_exclusions.episode_indices must be a subset of episode_indices")
            else:
                prompt_owned = set()
    ...
    merged = sorted(existing | set(bad))
    prompt_payload = {
        **previous_prompt,
        "reason": prompt_reason,
        "episode_indices": sorted(prompt_owned | set(bad)),
        "latest_scan": {
    ...
    payload = {
        **previous,
        ...
        "droid_prompt_exclusions": prompt_payload,
    }
    if args.dry_run:
        logger.info(
