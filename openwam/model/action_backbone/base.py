...
Three-level hierarchy, because the two architecture families drive the action
stream through genuinely different contracts:

- :class:`ActionBackbone` — the shared root every action backbone exposes
  (scheduler, normalization-shift, proprio/latent-decoder flags). Concrete
  subclasses inherit so the state_dict lives at ``action_backbone.<param>`` with
  no wrapping prefix.
...
class SharedActionBackbone(ActionBackbone):
...
class ActionDiTBackbone(ActionBackbone):
...
__all__ = ["ActionBackbone", "ActionDiTBackbone", "SharedActionBackbone"]
