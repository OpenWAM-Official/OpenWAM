...
class _DecoderBlock(nn.Module):
    ...
    def forward(self, query: torch.Tensor, latent: torch.Tensor) -> torch.Tensor:
        # Learnable queries carry their own position, so self-attn uses no RoPE.
        query = query + self.self_attn(self.self_attn_norm(query))
        query = query + self.cross_attn(self.cross_attn_norm(query), latent)
...
class LatentQueryDecoder(nn.Module):
    ...
    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        """latent (B, T_latent, latent_dim) -> action (B, num_query, real_action_dim)."""
        latent_h = self.latent_proj(latent)
        query = self.query.unsqueeze(0).expand(latent.shape[0], -1, -1)
        for block in self.blocks:
            query = block(query, latent_h)
