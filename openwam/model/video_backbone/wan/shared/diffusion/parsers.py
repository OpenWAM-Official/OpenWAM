...
def add_dataset_base_config(parser: argparse.ArgumentParser):
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Base path of the dataset (required for unified dataset type).",
    )
    parser.add_argument(
...
def add_training_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--max_steps", type=int, default=None, help="Max training steps. Overrides --num_epochs when set."
    )
    parser.add_argument("--batch_size", type=int, default=1, help="Per-GPU batch size. Default 1 (legacy behavior).")
    parser.add_argument(
        "--trainable_models", type=str, default=None, help="Models to train, e.g., dit, vae, text_encoder."
    )
    parser.add_argument(
        "--find_unused_parameters", default=False, action="store_true", help="Whether to find unused parameters in DDP."
    )
...
def add_output_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="High-frequency rolling checkpoint interval. Only the last K are kept (see --keep_last_k_ckpts).",
    )
    parser.add_argument(
        "--keep_last_k_ckpts", type=int, default=2, help="Number of rolling checkpoints to keep. Default 2."
    )
    return parser
...
def add_wandb_config(parser: argparse.ArgumentParser):
    parser.add_argument(
        "--wandb_project", type=str, default=None, help="Wandb project name. If set, enables wandb logging."
    )
