...
def _move_cosmos_vae(vae: Any, *, dtype: torch.dtype, device: torch.device) -> None:
    ...
    # 1.0 / self.std]` (wan2pt1.py:764) — a plain Python list that captured
    # the original tensors. The `setattr` loop above rebinds `outer.mean` and
    # `outer.std` to moved tensors but leaves the list still pointing at the
    # stale cuda:0 references; `encode()` (line 532) then mixes the cuda:N
    # latents with the cuda:0 scale and crashes "Expected all tensors to be on
    # the same device". Rebuild the list from the freshly-moved tensors,
    # mirroring the upstream init pattern exactly so behaviour is identical.
...
def _move_cosmos_reason1(te: Any, *, dtype: torch.dtype, device: torch.device) -> None:
    ...
    Mirrors :func:`_move_cosmos_vae` — the encoder facade is intentionally
    not an ``nn.Module``. The wrapper separately registers the inner Qwen
    module as ``_reason1_inner`` so the weights still enter ``state_dict()``,
    but dtype/device bookkeeping lives on the facade. Delegate to the
    encoder's own ``to`` shim to keep both in sync.
    """
    if te is None:
        return
