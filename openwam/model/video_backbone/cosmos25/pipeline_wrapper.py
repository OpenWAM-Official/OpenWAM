...
- ``state.hidden_states`` : ``(B, T, H, W, D)`` patchified hidden state. Load-bearing.
- ``state.context``       : ``(B, L, context_dim)`` cross-attn key/value source. Load-bearing.
- ``state.grid_frames`` / ``state.grid_height`` / ``state.grid_width`` : patch grid
  ``(T, H, W)`` the architecture reads for mask construction / unpatchify.
...
- ``state.time_mod`` / ``state.rope_freqs`` : Wan-style dummies. Architectures using
