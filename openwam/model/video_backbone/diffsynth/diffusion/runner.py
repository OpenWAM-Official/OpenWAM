...
def launch_training_task(
    ...
    if batch_size > 1:
        dataloader = torch.utils.data.DataLoader(
            dataset, shuffle=True, batch_size=batch_size, collate_fn=lambda x: x, num_workers=num_workers
        )
    else:
        dataloader = torch.utils.data.DataLoader(
        ...
        for data in tqdm(dataloader):
            with accelerator.accumulate(model):
                ...
                model_logger.on_step_end(
                    accelerator, model, save_steps, loss=loss, learning_rate=current_lr, **extra_log_kwargs
                )
                scheduler.step()
