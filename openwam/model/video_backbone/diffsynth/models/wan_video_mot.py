...
from .wan_video_dit import MLP, DiTBlock, SelfAttention, flash_attention, modulate, rope_apply
...
class MotWanAttentionBlock(DiTBlock):
    ...
    def forward(self, wan_block, x, context, t_mod, freqs, x_mot, context_mot, t_mod_mot, freqs_mot):
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            wan_block.modulation.to(dtype=t_mod.dtype, device=t_mod.device) + t_mod
        ).chunk(6, dim=1)
...
class MotWanModel(torch.nn.Module):
    def __init__(
        ...
        self.blocks = torch.nn.ModuleList(
            [MotWanAttentionBlock(has_image_input, dim, num_heads, ffn_dim, eps, block_id=i) for i in self.mot_layers]
        )
