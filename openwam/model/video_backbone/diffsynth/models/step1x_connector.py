...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t):
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size, self.max_period).type(t.dtype)  # type: ignore
...
class Qwen2Connector(torch.nn.Module):
    ...
    def forward(self, x, t, mask):
        mask_float = mask.unsqueeze(-1)  # [b, s1, 1]
        x_mean = (
            (x * mask_float).sum(dim=1)
            / mask_float.sum(dim=1)
            * (1 + self.scale_factor.to(dtype=x.dtype, device=x.device))
        )
