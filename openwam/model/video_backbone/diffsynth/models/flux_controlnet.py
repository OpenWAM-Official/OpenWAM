...
from .flux_dit import FluxJointTransformerBlock, FluxSingleTransformerBlock, RMSNorm, RoPEEmbedding, TimestepEmbeddings
...
class FluxControlNet(torch.nn.Module):
    def __init__(
        ...
        self.single_blocks = torch.nn.ModuleList(
            [FluxSingleTransformerBlock(3072, 24) for _ in range(num_single_blocks)]
        )

        self.controlnet_blocks = torch.nn.ModuleList([torch.nn.Linear(3072, 3072) for _ in range(num_joint_blocks)])
        self.controlnet_single_blocks = torch.nn.ModuleList(
            [torch.nn.Linear(3072, 3072) for _ in range(num_single_blocks)]
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        controlnet_res_stack = self.align_res_stack_to_original_blocks(
            controlnet_res_stack, 19, hidden_states[:, prompt_emb.shape[1] :]
        )
        controlnet_single_res_stack = self.align_res_stack_to_original_blocks(
            controlnet_single_res_stack, 38, hidden_states[:, prompt_emb.shape[1] :]
        )
