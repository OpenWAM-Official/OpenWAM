...
class Step1xEditEmbedder(torch.nn.Module):
    def __init__(
        self, model: QwenImageTextEncoder, processor, max_length=640, dtype=torch.bfloat16, device=get_device_type()
    ):
        super().__init__()
...
User Prompt:"""
    ...
    def forward(self, caption, ref_images):
        ...
        for idx, (txt, imgs) in enumerate(zip(text_list, ref_images)):
            ...
            inputs.input_ids = (
                torch.cat([old_inputs_ids[0, :idx1], new_txt_ids[0, idx2:]], dim=0).unsqueeze(0).to(get_device_type())
            )
