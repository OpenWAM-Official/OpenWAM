            ...
            for name in list(state_dict_.keys()):
                if "single_blocks." in name and ".a_to_q." in name:
                    ...
                    if "lora_A" in name:
                        ...
                        param = torch.zeros(
                            (3 * d + mlp.shape[0], 3 * r + mlp.shape[1]),
                            dtype=state_dict_[name].dtype,
                            device=state_dict_[name].device,
                        )
                ...
                for component in ["a", "b"]:
                    if f".{component}_to_q." in name:
                        ...
                        if "lora_A" in name:
                            ...
                            param[d : 2 * d, r : 2 * r] = state_dict_[
                                name.replace(f".{component}_to_q.", f".{component}_to_k.")
                            ]
                            param[2 * d : 3 * d, 2 * r : 3 * r] = state_dict_[
                                name.replace(f".{component}_to_q.", f".{component}_to_v.")
                            ]
                        else:
                            param = torch.concat(
                                [
...
class LoraMerger(torch.nn.Module):
    ...
    def forward(self, base_output, lora_outputs):
        ...
        gate = self.activation(
            ...
            + norm_base_output * norm_lora_outputs * self.weight_cross
            + self.bias
        )
...
class FluxLoraPatcher(torch.nn.Module):
    ...
    def default_lora_patterns(self):
        ...
        lora_dict = {
            ...
            "ff_a.0": 12288,
            "ff_a.2": 3072,
            "norm1_a.linear": 18432,
            "attn.b_to_qkv": 9216,
            "attn.b_to_out": 3072,
            "ff_b.0": 12288,
            "ff_b.2": 3072,
            "norm1_b.linear": 18432,
        }
        for i in range(19):
            for suffix in lora_dict:
                lora_patterns.append({"name": f"blocks.{i}.{suffix}", "dim": lora_dict[suffix]})
        lora_dict = {"to_qkv_mlp": 21504, "proj_out": 3072, "norm.linear": 9216}
        for i in range(38):
            for suffix in lora_dict:
                lora_patterns.append({"name": f"single_blocks.{i}.{suffix}", "dim": lora_dict[suffix]})
