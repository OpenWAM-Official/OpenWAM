...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            cos_sim = torch.cosine_similarity(masked_image_embeds, image_embeddings, dim=-1)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            model_inputs.update(
                self.prepare_prefilled_image_embeds(
                    len(output_image_embeddings), num_img_tokens, is_sampling_img, **model_kwargs
                )
            )

            # parse position_ids from model_kwargs
            model_inputs.update(
                self.prepare_image_position_ids(input_ids, generation_image_grid_thw, is_sampling_img, **model_kwargs)
            )
        ...
        output_image_embeddings = (
            torch.cat(output_image_embeddings, dim=1) if len(output_image_embeddings) > 0 else None
        )
    ...
    def get_default_image_grid_thw(
        self,
    ):
