...
class NexusGenAutoregressiveModel(torch.nn.Module):
    ...
    def get_editing_msg(self, instruction):
        ...
        messages = [
            ...
            {"role": "assistant", "content": "Here is the image: <image>"},
        ]
        return messages
    ...
    def forward(self, instruction, ref_image=None, num_img_tokens=81):
        ...
        if ref_image is not None:
            ...
            output_image_embeddings = self.get_target_embeddings(
                images, messages, self.processor, self.model, num_img_tokens
            )
        else:
    ...
    def get_target_embeddings(self, images, messages, processor, model, num_img_tokens=81):
        ...
        input_image_mask = torch.logical_and(
            indices <= (image_embeds.shape[0] - ground_truth_image_embeds.shape[0]), image_mask
        )
        ...
        image_prefill_embeds = model.image_prefill_embeds(torch.arange(81, device=model.device).long())
        input_embeds = input_embeds.masked_scatter(
            gt_image_mask.unsqueeze(-1).expand_as(input_embeds), image_prefill_embeds
        )

        position_ids, _ = model.get_rope_index(
            inputs["input_ids"], inputs["image_grid_thw"], attention_mask=inputs["attention_mask"]
        )
