...
class WanVideoUnit_SpeedControl(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("motion_bucket_id",), output_params=("motion_bucket_id",))
...
class WanVideoUnit_VAP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        if inputs_shared.get("vap_video") is None:
            ...
            vap_prompt, negative_vap_prompt = (
                inputs_posi.get("vap_prompt", ""),
            ...
            if end_image is not None:
                ...
                if pipe.dit.has_image_pos_emb:
                    vap_clip_context = torch.concat(
                        [vap_clip_context, pipe.image_encoder.encode_image([vap_end_image])], dim=1
                    )
            ...
            if end_image is not None:
                ...
                vae_input = torch.concat(
                    [
                        ...
                        torch.zeros(3, num_frames - 2, height, width).to(image_vap.device),
                ...
                vae_input = torch.concat(
                    [image_vap.transpose(0, 1), torch.zeros(3, num_frames - 1, height, width).to(image_vap.device)],
                    dim=1,
                )
            ...
            vap_latent = pipe.vae.encode(
                vap_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            ).to(dtype=pipe.torch_dtype, device=pipe.device)
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_motion_latents(
        ...
        if motion_video is not None:
            assert motion_video.shape[2] == motion_frames, (
            ...
            motion_latents = torch.zeros(
                [1, 3, motion_frames, height, width], dtype=pipe.torch_dtype, device=pipe.device
            )
            kwargs["drop_motion_frames"] = True
        motion_latents = pipe.vae.encode(
            motion_latents, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ).to(dtype=pipe.torch_dtype, device=pipe.device)
    ...
    def process_pose_cond(
        ...
        for r in range(num_repeats):
            ...
            cond_latents = pipe.vae.encode(
                cond, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            ).to(dtype=pipe.torch_dtype, device=pipe.device)
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        if (
            (inputs_shared.get("input_audio") is None and inputs_shared.get("audio_embeds") is None)
        ...
        inputs_shared.update(
            self.process_motion_latents(pipe, height, width, tiled, tile_size, tile_stride, motion_video)
        )
        inputs_shared.update(
    ...
    def pre_calculate_audio_pose(
        ...
        assert pipe.audio_encoder is not None and pipe.audio_processor is not None, (
...
class WanVideoUnit_AnimatePoseLatents(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, animate_pose_video, tiled, tile_size, tile_stride):
        ...
        pose_latents = pipe.vae.encode(
            animate_pose_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ).to(dtype=pipe.torch_dtype, device=pipe.device)
...
class WanVideoUnit_AnimateInpaint(PipelineUnit):
    ...
    def process(
        ...
        y_reft = pipe.vae.encode(
            bg_pixel_values, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ...
        ref_latents = pipe.vae.encode(
            ref_pixel_values, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ).to(dtype=pipe.torch_dtype, device=pipe.device)
...
class WanVideoUnit_LongCatVideo(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("longcat_video",), output_params=("longcat_latents",), onload_model_names=("vae",)
        )
...
def model_fn_wan_video(
    ...
    bridge_feature_store: Optional[list] = None,
    ...
    **kwargs,
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if (
                bridge_feature_store is not None
                and bridge_feature_layers is not None
                and block_id in bridge_feature_layers
            ):
            ...
            if action_dit_state is not None and block_id in action_dit_state.action_dit.bridge_layers_set:
                _adi = action_dit_state.action_dit
                _i = action_dit_state.bridge_block_counter

                # Project video hidden state down to action dim
                _skip = action_dit_state.skip_prefix_tokens
                x_for_action = x[:, _skip:, :] if _skip > 0 else x

                x_video_proj_i = _adi.video_projs[_i](x_for_action)
                if action_dit_state.x_video_proj is None:
                    action_dit_state.x_video_proj = x_video_proj_i
                else:
                    action_dit_state.x_video_proj = action_dit_state.x_video_proj + x_video_proj_i

                # Run JointActionDiTBlock[i]
                _block = _adi.blocks[_i]
                _x_action = action_dit_state.x_action
                _x_video = action_dit_state.x_video_proj
                _t_mod_a = action_dit_state.t_mod

                def _create_joint_forward(joint_block):
                    def _fwd(*args):
                        return joint_block(*args)

                    return _fwd

                if action_dit_state.use_gradient_checkpointing and _adi.training:
                    if action_dit_state.use_gradient_checkpointing_offload:
                        with torch.autograd.graph.save_on_cpu():
                            _x_action, _x_video = torch.utils.checkpoint.checkpoint(
                                _create_joint_forward(_block),
                                _x_action,
                                _x_video,
                                _t_mod_a,
                                use_reentrant=False,
                    ...
                    _x_action, _x_video = _block(_x_action, _x_video, _t_mod_a)

                # Back-project to video dim and add as residual
                _back_proj = _adi.video_back_projs[_i](_x_video)
                if _skip > 0:
                    # Only add residual to the non-ref portion of x
                    x = torch.cat([x[:, :_skip, :], x[:, _skip:, :] + _back_proj], dim=1)
                else:
                    x = x + _back_proj

                # Update state
                action_dit_state.x_action = _x_action
                action_dit_state.x_video_proj = _x_video
                action_dit_state.bridge_block_counter = _i + 1
    ...
    if shared_backbone_state is not None:
        ...
        shared_backbone_state.action_noise_pred = shared_backbone_state._architecture.extract_action_prediction(
            _action_state
        )
