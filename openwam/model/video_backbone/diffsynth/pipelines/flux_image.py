    ...
    def from_pretrained(
        ...
        if value_controllers is not None:
            ...
            if hasattr(pipe.value_controller.encoders[0], "vram_management_enabled"):
                pipe.value_controller.vram_management_enabled = pipe.value_controller.encoders[
                    0
    ...
    def process(self, pipe: FluxImagePipeline, prompt, t5_sequence_length, positive) -> dict:
        if pipe.text_encoder_1 is not None and pipe.text_encoder_2 is not None:
            prompt_emb, pooled_prompt_emb, text_ids = self.encode_prompt(
                tokenizer_1=pipe.tokenizer_1,
                tokenizer_2=pipe.tokenizer_2,
                text_encoder_1=pipe.text_encoder_1,
...
class FluxImageUnit_IPAdapter(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        ipadapter_images, ipadapter_scale = (
            inputs_shared.get("ipadapter_images", None),
            inputs_shared.get("ipadapter_scale", 1.0),
        )
...
class FluxImageUnit_EntityControl(PipelineUnit):
    ...
    def preprocess_masks(self, pipe, masks, height, width, dim):
        ...
        for mask in masks:
            mask = (
                pipe.preprocess_image(mask.resize((width, height), resample=Image.NEAREST)).mean(dim=1, keepdim=True)
                > 0
            )
    ...
    def prepare_eligen(
        ...
        if enable_eligen_on_negative and cfg_scale != 1.0:
            entity_prompt_emb_nega = (
                prompt_emb_nega["prompt_emb"].unsqueeze(1).repeat(1, entity_masks_posi.shape[1], 1, 1)
            )
    ...
    def process(self, pipe: FluxImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        eligen_entity_prompts, eligen_entity_masks = (
            inputs_shared.get("eligen_entity_prompts", None),
            inputs_shared.get("eligen_entity_masks", None),
        )
...
class FluxImageUnit_NexusGen(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        if inputs_shared.get("nexus_gen_reference_image", None) is None:
            ...
            inputs_posi["text_ids"] = self.get_editing_text_ids(
                ...
                embeds_grid[0][1].item(),
                embeds_grid[0][2].item(),
                ref_embeds_grid[0][1].item(),
                ref_embeds_grid[0][2].item(),
            )
        return inputs_shared, inputs_posi, inputs_nega
    ...
    def get_editing_text_ids(self, latents, target_embed_height, target_embed_width, ref_embed_height, ref_embed_width):
        ...
        ref_embed_ids = (
            ref_embed_ids[None, :].repeat(batch_size, 1, 1, 1).reshape(batch_size, height // 2 * width // 2, 3)
        )
...
class InfinitYou(torch.nn.Module):
    ...
    def prepare_infinite_you(self, model, id_image, infinityou_guidance, device):
        ...
        landmark = sorted(face_info, key=lambda x: (x["bbox"][2] - x["bbox"][0]) * (x["bbox"][3] - x["bbox"][1]))[-1][
            "kps"
    ...
    **kwargs,
    ...
    if tiled:
        ...
        def flux_forward_fn(hl, hr, wl, wr):
            tiled_controlnet_conditionings = (
                [f[:, :, hl:hr, wl:wr] for f in controlnet_conditionings]
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if controlnet is not None and controlnet_conditionings is not None and controlnet_res_stack is not None:
                if kontext_latents is None:
                    ...
                    hidden_states[:, : -kontext_latents.shape[1]] = (
                        hidden_states[:, : -kontext_latents.shape[1]] + controlnet_res_stack[block_id]
                    )
        ...
        for block_id, block in enumerate(dit.single_blocks):
            ...
            if (
                ...
                and controlnet_conditionings is not None
                and controlnet_single_res_stack is not None
            ):
                if kontext_latents is None:
                    hidden_states[:, prompt_emb.shape[1] :] = (
                        hidden_states[:, prompt_emb.shape[1] :] + controlnet_single_res_stack[block_id]
                    )
                else:
                    hidden_states[:, prompt_emb.shape[1] : -kontext_latents.shape[1]] = (
                        hidden_states[:, prompt_emb.shape[1] : -kontext_latents.shape[1]]
                        + controlnet_single_res_stack[block_id]
                    )
