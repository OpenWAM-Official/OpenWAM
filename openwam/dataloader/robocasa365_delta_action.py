"""RoboCasa365DeltaAction compact LeRobot v3 reader.
...
STATE_DIM = 19
ACTION_DIM = 15
ACTION_STATS_KEY = "robocasa365_delta_action"
STATE_STATS_KEY = "robocasa365_delta_action_state"
...
def _task_dir_name(data_root: str) -> str:
    return os.path.basename(data_root.rstrip("/")) or "robocasa365_delta_action"
...
class RoboCasa365DeltaActionDataset(BaseDataset):
    """One task view into an aggregated compact RoboCasa365DeltaAction LeRobot v3 repo."""
    ...
    def __init__(
        ...
        **_unused,
        ...
        if info.get("codebase_version") != "v3.0":
            raise ValueError(f"RoboCasa365DeltaAction compact reader requires LeRobot v3.0, got {info.get('codebase_version')!r}")
        ...
        if state_shape != (STATE_DIM,) or action_shape != (ACTION_DIM,):
            raise ValueError(
                f"RoboCasa365DeltaAction compact reader requires state{STATE_DIM}/action{ACTION_DIM}; got {state_shape}/{action_shape}. "
                "Run scripts/convert_robocasa365_delta_action_v3.py first."
            )
        ...
        if missing_camera_features:
            raise ValueError(
                "RoboCasa365DeltaAction video feature(s) required by the configured view layout are missing: "
        ...
        if missing_video_columns:
            raise ValueError(f"RoboCasa365DeltaAction episode metadata is missing required video columns: {missing_video_columns}")
        ...
        print(f"RoboCasa365DeltaActionDataset[{self.task_name}]: {len(self._ep_pos)} episodes ({split})")
        ...
        if self.normalize_mode is not None:
            ...
            if not os.path.isfile(stats_path):
                raise FileNotFoundError(
                    f"compact RoboCasa365DeltaAction stats not found: {stats_path}; conversion writes meta/normalization_stats.npy"
                )
    ...
    def _get_prompt(self, local_index: int) -> str:
        ...
        # RoboCasa365DeltaAction uses the native LeRobot task text without a wrapper.
...
class MultiTaskRoboCasa365DeltaActionDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not roots:
            raise FileNotFoundError(f"No RoboCasa365DeltaAction tasks found under {dataset_dir}")
        if normalize_mode is not None and normalization_stats_path is None and len({repo for _, repo in roots}) > 1:
            raise ValueError("multi-repo RoboCasa365DeltaAction requires normalization_stats_path")
        print(f"MultiTaskRoboCasa365DeltaActionDataset: {len(roots)} task bucket(s)")
        self._datasets = [
            RoboCasa365DeltaActionDataset(
                data_root=repo,
                task_name=name,
                normalize_mode=normalize_mode,
...
__all__ = [
    ...
    "HEAD_CAMERA",
    "MultiTaskRoboCasa365DeltaActionDataset",
    "RIGHT_CAMERA",
    "RoboCasa365DeltaActionDataset",
    "STATE_DIM",
    "STATE_STATS_KEY",
