...
ROBODOJO_SOURCE_FRAME = "env_origin_relative_position_world_orientation_wxyz"
...
_CAMERA_DATASETS = {
    camera: f"vision/{camera}/colors" for camera in DEFAULT_ROBODOJO_CAMERA_LAYOUT
}
...
def calibration_fingerprint(calibration: Mapping[str, Any]) -> str:
    """Return SHA-256 of the validated canonical calibration JSON."""
    canonical = validate_calibration(calibration)
...
def read_calibrated_eef20(
    ...
    """Read one episode's achieved states through the canonical EEF20 path.
    ...
    if isinstance(episode, (str, Path)):
        with h5py.File(episode, "r") as handle:
            return read_calibrated_eef20(handle, calibration, start, end)
...
def validate_robodojo_episode(path: str | Path) -> dict[str, Any]:
    ...
    with h5py.File(episode_path, "r") as handle:
        ...
        for key in _POSE_KEYS:
            ...
            if len(shape) != 2 or shape[1] != 7:
                raise ValueError(
                    f"{episode_path}:{key} must have exact shape (T, 7), got {shape}"
                )
        for key in _GRIPPER_KEYS:
            shape = handle[key].shape
            if len(shape) != 2 or shape[1] != 1:
                raise ValueError(
                    f"{episode_path}:{key} must have exact shape (T, 1), got {shape}"
                )
        ...
        lengths = {
            key: int(handle[key].shape[0])
            for key in (*_POSE_KEYS, *_GRIPPER_KEYS, *_CAMERA_DATASETS.values())
        }
        unique_lengths = set(lengths.values())
        if len(unique_lengths) != 1 or 0 in unique_lengths:
            raise ValueError(
                f"{episode_path}: all state and camera datasets must have equal non-zero T; "
                f"got {lengths}"
            )
        length = next(iter(unique_lengths))
        if length < 2:
            raise ValueError(
                f"{episode_path}: RoboDojo episodes need at least two frames, got T={length}"
            )
        ...
        for key in _POSE_KEYS:
            ...
            if not np.all(
                np.isclose(norms, 1.0, rtol=0.0, atol=_QUATERNION_ATOL)
            ):
                bad_index = int(
                    np.flatnonzero(
                        ~np.isclose(
        ...
        if instruction_dataset.shape != ():
            raise ValueError(
                f"{episode_path}:instruction must be scalar, got shape "
                f"{instruction_dataset.shape}"
            )
...
def _requested_tasks_exist(available: Sequence[str], requested: Sequence[str]) -> None:
    ...
    if missing:
        raise FileNotFoundError(
            "requested RoboDojo task(s) are missing from the formal dataset root: "
            + ", ".join(missing)
        )
...
def resolve_robodojo_tasks(
    ...
    *,
    ...
    """Resolve single-task, allowlist, and holdout selection consistently."""
    if split not in {"train", "val"}:
        raise ValueError(f"split must be 'train' or 'val', got {split!r}")
    available = discover_robodojo_tasks(dataset_dir, embodiment=embodiment)
    if not available:
        raise FileNotFoundError(
            f"no formal RoboDojo task directories found in {dataset_dir}"
        )
    ...
    train_requested = (
        None if train_tasks is None else [str(task) for task in train_tasks]
    )
    holdout_requested = (
        [] if holdout_tasks is None else [str(task) for task in holdout_tasks]
    )
    if train_requested is not None:
        _requested_tasks_exist(available, train_requested)
    _requested_tasks_exist(available, holdout_requested)

    if split == "val" and holdout_requested:
        selected = sorted(set(holdout_requested))
    else:
        selected = available if train_requested is None else sorted(set(train_requested))
        if split == "train" and holdout_requested:
            holdout = set(holdout_requested)
            selected = [task for task in selected if task not in holdout]
    if not selected:
        raise ValueError(
            f"RoboDojo task selection is empty for split={split!r}; "
            "check task_name/train_tasks/holdout_tasks"
        )
    return selected
...
def _load_validated_stats(
    ...
    try:
        ...
        raise ValueError(
            f"could not read RoboDojo normalization stats from {stats_path}"
        ) from error
    ...
    if not isinstance(mode_stats, Mapping):
        raise KeyError(
            f"{stats_path}: normalization stats must contain nested key "
            f"{DEPLOY_ACTION_MODE!r}"
        )
    ...
    for key in STAT_KEYS:
        ...
        if value.shape != (EEF20_DIM,):
            raise ValueError(
                f"{stats_path}:eef.{key} must have exact shape ({EEF20_DIM},), "
                f"got {value.shape}"
            )
        if value.dtype.kind not in "fiu" or not np.all(np.isfinite(value)):
            raise ValueError(
                f"{stats_path}:eef.{key} must contain only finite numeric values"
            )
    ...
    saved_fingerprint = metadata.get(
        ...
        mode_stats.get("calibration_fingerprint"),
    )
    expected_fingerprint = calibration_fingerprint(calibration)
    if not isinstance(saved_fingerprint, str):
        raise ValueError(
            f"{stats_path}: metadata.calibration_fingerprint is required"
        )
...
class RoboDojoDataset(BaseDataset):
    """One formal RoboDojo task rooted at ``<task>/arx_x5/data``."""
    ...
    def __init__(
        ...
        *,
        ...
        if action_mode != DEPLOY_ACTION_MODE:
            raise ValueError(
                f"RoboDojo supports only action_mode='eef', got {action_mode!r}"
            )
        ...
        self._video_sample_indices = list(
            range(0, self.num_frames, self.video_stride)
        )
        ...
        if self.multiview:
            if (
                len(self.camera_layout) != 3
                or set(self.camera_layout)
                != set(DEFAULT_ROBODOJO_CAMERA_LAYOUT)
            ):
                raise ValueError(
            ...
            raise ValueError(
                f"target_camera must be one of {list(_CAMERA_DATASETS)}, "
        ...
        if supplied_root.name == "data":
            ...
            if supplied_root.resolve() != expected_data_root.resolve():
                raise ValueError(
                    ...
                    "dataset_root/task_name/arx_x5/data; "
            ...
            if not task_name:
                raise ValueError(
                    "task_name is required when data_root is the RoboDojo dataset root"
                )
            formal_dataset_root = supplied_root
            if (
                dataset_root is not None
                and Path(dataset_root).resolve() != formal_dataset_root.resolve()
            ):
                raise ValueError(
        ...
        all_metadata = [
            validate_robodojo_episode(path) for path in episode_paths
        ]
        ...
        self._episode_lengths = [
            int(all_metadata[index]["length"]) for index in selected_indices
        ]
        self._instructions = [
            str(all_metadata[index]["instruction"]) for index in selected_indices
        ]
        ...
        if self._unify_action:
            if unify_action_map is None:
                raise ValueError(
                    "RoboDojo unify_action=true requires an explicit "
                    'unify_action_map; use ["0-9", "34-43"]'
                )
            ...
            if not np.array_equal(
                ...
                raise ValueError(
                    "RoboDojo requires the canonical unify_action_map "
                    '["0-9", "34-43"] exactly'
                )
        ...
        if self.normalize_mode is not None:
            if normalization_stats_path is None:
                raise ValueError(
                    "RoboDojo normalization requires an explicit "
                    "normalization_stats_path; runtime corpus scans are disabled"
                )
            stats_path = Path(normalization_stats_path)
            if not stats_path.is_file():
                raise FileNotFoundError(
                    f"RoboDojo normalization stats do not exist: {stats_path}"
                )
    ...
    def normalization_stats(self) -> dict[str, np.ndarray] | None:
        ...
        return {
            ...
            for key, value in self._mode_stats.items()
        }
    ...
    def _video_at(
        ...
        if self.multiview:
            frames = {
                camera: self._frame(handle, camera, frame_index)
                for camera in self.camera_layout
            }
    ...
    def _build_sample(self, episode_index: int, start: int) -> dict[str, Any]:
        ...
        if actual_length < 2:
            raise IndexError(
                f"RoboDojo window start={start} has no real next-state target "
                f"for episode length {episode_length}"
            )
        ...
        with h5py.File(path, "r") as handle:
            ...
            for relative_index in self._video_sample_indices:
                source_index = (
                    ...
                    if relative_index < actual_length
                    else episode_length - 1
                )
        ...
        time_validity = torch.tensor(
            [
                action_index + 1 < actual_length
                for action_index in range(self.num_action_steps)
            ],
            dtype=torch.bool,
        )
        ...
        action_mask = (
            time_validity[:, None]
            & dimension_validity[None, :]
        ).contiguous()
        proprio_mask = dimension_validity[None, :].clone()
        video_mask = torch.tensor(
            [
                relative_index < actual_length
                for relative_index in self._video_sample_indices
            ],
            dtype=torch.bool,
        )
        ...
        return {
            ...
            "prompt": format_prompt_for_inference(
                self._instructions[episode_index]
            ),
...
class MultiTaskRoboDojoDataset(BaseDataset):
    ...
    def from_config(cls, config, split: str = "train"):
        ...
        return cls(
            ...
            task_name=_config_get(config, "task_name", None),
            train_tasks=_config_get(config, "train_tasks", None),
            holdout_tasks=_config_get(config, "holdout_tasks", None),
            split=split,
    ...
    def __init__(
        ...
        **dataset_kwargs,
        ...
        if not self._sub_datasets or cumulative == 0:
            raise ValueError("RoboDojo multi-task selection produced an empty dataset")
    ...
    def normalization_stats(self) -> dict[str, np.ndarray] | None:
        ...
        return {
            key: np.asarray(value).copy()
            for key, value in self._normalization_stats_shared.items()
        }
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        previous = (
            0
            if dataset_index == 0
            else self._cumulative_lengths[dataset_index - 1]
        )
