...
class PolicyServer:
    ...
    def _init_policy(self):
        ...
        from openwam.deployment.policy import WAMPolicy
        ...
        from openwam.dataloader.robotwin_dataset import DEFAULT_MULTIVIEW_CAMERA_LAYOUT
    ...
    def _decode_obs(self, obs: dict) -> dict:
        ...
        - ``multiview=False``: use ``head_camera`` only, ``_crop_and_resize`` to
        ...
        if not self._multiview:
            ...
            obs["image"] = _crop_and_resize(head_pil, self._img_height, self._img_width)
        else:
...
def build_server_from_config(cfg, ckpt_dir: str, device: str = "cuda", embodiment: Optional[str] = None):
    ...
    """
    from openwam.deployment import JointInferenceEngine
