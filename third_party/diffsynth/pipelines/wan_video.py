...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        ...
        if (vace_video is not None or vace_video_mask is not None or vace_reference_image is not None) and pipe.vace is not None:
    ...
    **kwargs,
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        ...
        timestep = torch.concat([
            ...
            torch.ones((latents.shape[2] - num_clean, tokens_per_frame), dtype=latents.dtype, device=latents.device) * timestep
        ]).flatten()
