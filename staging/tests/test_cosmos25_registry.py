...
def test_cosmos25_names_registered():
    assert _VIDEO_BACKBONE_REGISTRY["cosmos25_predict_2b"] is Cosmos25VideoBackbone
    assert _VIDEO_BACKBONE_REGISTRY["cosmos25_predict_14b"] is Cosmos25VideoBackbone


def test_cosmos25_build_with_bad_path_raises_clear_error():
    ...
    """
    cfg = {"video_backbone": {"name": "cosmos25_predict_2b", "model_path": "/nonexistent"}}
    with pytest.raises((ImportError, NotImplementedError, FileNotFoundError)) as exc_info:
        build_video_backbone("cosmos25_predict_2b", cfg)
    message = str(exc_info.value).lower()
    assert any(token in message for token in ("cosmos", "nonexistent", "variant"))


def test_cosmos25_register_idempotency_guard():
...
def test_cosmos25_invalid_sac_mode_rejected():
...
def test_cosmos25_invalid_vae_choice_rejected():
...
def test_cosmos25_invalid_text_encoder_rejected():
...
def test_cosmos25_text_encoder_reason1_live_requires_path():
...
def test_cosmos25_text_encoder_dropout_out_of_range_rejected():
    """`text_encoder_dropout` outside [0, 1] must fail-fast at build time
    (CPU CI testable). Mirrors the wrapper-side ValueError in
    `Cosmos25PipelineWrapper.__init__`; catching it at the builder avoids any
...
def test_cosmos25_text_encoder_dropout_with_text_encoder_none_rejected():
