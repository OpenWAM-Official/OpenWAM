#!/usr/bin/env bash
# Evaluate the RoboCasa365 native-delta policy on a task list.
...
#   bash scripts/deploy.sh --ckpt-dir /path/to/robocasa365_delta_action_ckpt --port 8848
#
# Then, inside the robocasa365 env:
#   ROBOCASA365_DELTA_ACTION_PYTHON=/path/to/python bash multi_eval.sh [options] <tasks...>
#
...
#   --out   <dir> results directory          (default: ./results_robocasa365_delta_action)
#   -h, --help
set -euo pipefail
...
split="target"; port="8848"; host="127.0.0.1"; out="./results_robocasa365_delta_action"
tasks=()
while [[ $# -gt 0 ]]; do
  case "$1" in
...
for task in "${TASKS[@]}"; do
  ...
  echo "--- ${task} ---"
  set +e
  ROBOCASA365_DELTA_ACTION_PORT="$port" ROBOCASA365_DELTA_ACTION_POLICY_HOST="$host" \
