...
class BehaviorBridge:
    ...
    def __init__(
        ...
        *,
        ...
        self._south = south_client if south_client is not None else WSPolicyClient(self._ws_url, timeout=request_timeout)
        self._step = 0
    ...
    def _resolve_prompt(self, obs: dict) -> str:
        ...
        ``cfg.prompt`` is non-null) → de-underscored activity name from the
        ``task_id`` mapping → ``default_prompt``. Fails fast otherwise so a
        language-conditioned checkpoint is never silently fed an empty prompt.
        """
        p = obs.get("prompt")
        ...
        raise ValueError(
            ...
            "Pass --task-names <json> (task_id→activity_name, generated from "
            "omnigibson.learning.utils.eval_utils.TASK_INDICES_TO_NAMES) or --default-prompt."
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--task-names",
        default=None,
        help="JSON file mapping task_id→activity_name (from eval_utils.TASK_INDICES_TO_NAMES); "
        "de-underscored into the language prompt.",
    )
    parser.add_argument("--default-prompt", default=None, help="fallback prompt when task_id is unmapped")
    parser.add_argument("--no-send-state", action="store_true", help="do not send proprio (non-proprio checkpoints only)")
