...
#        bash scripts/deploy.sh <ckpt_dir>            # port 8848
#   2. A GenManip eval server (local ray_eval_server.py with a job submitted
#      via `gmp submit ebench/generalist/test_mini --run_id X`), or an online
#      endpoint from `gmp online submit` (then set TOKEN and RUN_ID=task_id).
...
# Per-episode results are mirrored into ./client_results (override with GENMANIP_RESULT_DIR).
set -euo pipefail
cd "$(dirname "$0")/../.."
