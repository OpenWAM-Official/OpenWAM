...
- **Better load balancing through finer-grained parallelism:** Workers claim
  small trial batches, reducing idle time when some benchmark tasks take longer.
...
`LIBERO_PYTHON` points to the **separate LIBERO environment's Python**.
...
```bash
# LIBERO simulator Python, called automatically by the Worker
export LIBERO_PYTHON=/path/to/libero/bin/python
export LIBERO_PATH=/path/to/LIBERO
# OpenWAM Python (active environment)
python -m pip install -r benchmarks/libero/requirements-labtasker.txt
```
...
```bash
python benchmarks/libero/labtasker_submit.py --auto-start-local-server \
...
uses the configured `LIBERO_PYTHON` for the simulator. For a single GPU,
run only the first line.

```bash
python benchmarks/libero/labtasker_worker.py --operation build_manifest --gpu 0 &
python benchmarks/libero/labtasker_worker.py --operation build_manifest --gpu 1 &
python benchmarks/libero/labtasker_worker.py --operation build_manifest --gpu 2 &
python benchmarks/libero/labtasker_worker.py --operation build_manifest --gpu 3 &
```
...
This uses all four suites, 50 trials per task and batches of 5.
...
Ports default to `8920 + GPU ID`, so these four Workers use ports 8920–8923.

```bash
python benchmarks/libero/labtasker_worker.py --gpu 0 &
python benchmarks/libero/labtasker_worker.py --gpu 1 &
python benchmarks/libero/labtasker_worker.py --gpu 2 &
python benchmarks/libero/labtasker_worker.py --gpu 3 &
```
...
python benchmarks/libero/labtasker_summarize.py \
  SUBMISSION_ID --output-dir outputs/libero/summary
```

The summary reads Labtasker Task records only and writes `task_summary.csv` and
`suite_summary.csv`. Check the task counts before using a success rate as a final
score: incomplete runs report partial results. Each successful eval Task also
returns an `artifact_dir` for its detailed files.
...
The cache defaults to `outputs/manifest-cache/libero`. Override it with
...
A 50-entry cache can serve a shorter `--num-trials` prefix. A missing cache is
built normally by `build_manifest`; an existing invalid or too-short cache
requires `--rebuild`.

For a smoke run, add `--smoke` to both build and eval submissions.
Use `--task-ids all|0,1` to select tasks, `--num-trials N` to request an
ordered prefix, and `--note TEXT` to append a display-name suffix. The official
default remains all four suites, all ten tasks per suite, and 50 trials per
task; smaller values are for debug or small-scale evaluation.
...
Logs, videos and results default to `outputs/libero/<route>`; override with
