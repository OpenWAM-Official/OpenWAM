...
def import_smoke() -> None:
    ...
    # LIBERO reads LIBERO_CONFIG_PATH at import time, so set it after the config
    # has been materialized and before importing the package.
    ...
    suite_names = sorted(task_map.keys())
...
def task_smoke(suite: str, task_id: int) -> None:
    ...
    if suite not in task_map:
        available = ", ".join(sorted(task_map))
        raise SystemExit(f"Unknown suite '{suite}'. Available suites: {available}")
...
def env_smoke(suite: str, task_id: int, camera_size: int, steps: int) -> None:
    ...
    try:
        ...
        print(f"env_smoke=ok reward={reward} done={done} obs_keys={','.join(sorted(obs.keys()))}")
    finally:
        env.close()
