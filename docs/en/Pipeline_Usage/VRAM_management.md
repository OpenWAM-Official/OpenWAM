...
> 
> A: Native FP8 computation is only supported on Hopper-architecture GPUs and has significant computational errors. We currently do not enable FP8 precision computation. The current FP8 quantization only reduces VRAM usage but does not improve computation speed.
