...
def test_parse_episode_stats_distinguishes_step_limit_hits(tmp_path):
    log = tmp_path / "adjust_bottle_demo_clean.log"
    body = (
    ...
    episodes, step_limit_hits = export_results_csv.parse_episode_stats(log)
    assert episodes == 3
    assert step_limit_hits == 1
    assert export_results_csv.parse_success_rate(log) == pytest.approx(33.3)


def test_parse_episode_stats_missing_log(tmp_path):
    assert export_results_csv.parse_episode_stats(tmp_path / "nope.log") == (0, 0)


def test_parse_episode_stats_fail_without_step_line(tmp_path):
    # A Fail! with no preceding step line must not be miscounted as a limit hit.
    log = tmp_path / "x_demo_clean.log"
    _write_log(log, f"{_R}Fail!{_X}\n" + _rate(0, 1))
    assert export_results_csv.parse_episode_stats(log) == (1, 0)
