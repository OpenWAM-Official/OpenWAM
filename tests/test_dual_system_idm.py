...
def test_idm_generate_rejects_no_video_step_without_input_latents(monkeypatch):
    ...
    with pytest.raises(ValueError, match="no video denoising step"):
        arch.generate(
            schedule=[(0.0, 1.0), (0.0, 0.5), (0.0, 0.0)],
            prompt="",
            num_frames=3,
            action_num_frames=5,
            decode_video=False,
...
def test_idm_generate_stage1_uses_dit_cache_skip_path(monkeypatch):
    """IDM's overridden generate should honor the same DiT cache API as the base path."""
    monkeypatch.setattr(torch.compiler, "cudagraph_mark_step_begin", lambda: None)
    vb = _GenerateVideoBackbone()
    arch = _make_idm_with_video(vb)
    arch.eval()
    cache = _ReuseOnceVideoCache()

    result = arch.generate(
        schedule=[(3.0, 1.0), (2.0, 1.0), (1.0, 0.5), (0.0, 0.0)],
        prompt="",
        num_frames=3,
        action_num_frames=5,
        decode_video=False,
    ...
    assert result["video"] is None
    assert result["actions"].shape == (4, 3)
    assert cache.get_cached_calls == 1
    assert len(cache.update_calls) == 2
    # Two stage-1 recomputes plus one stage-2 video-cache prefill.
    assert vb.prepare_count == 3
