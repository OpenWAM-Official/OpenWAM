...
def test_openwam_trainer_uses_strategy_freeze_for_tri_system_vlm(monkeypatch):
    ...
    monkeypatch.setattr(
        "openwam.train.openwam_trainer.OpenWAMTrainer._zero3_init_disabled",
        staticmethod(lambda: __import__("contextlib").nullcontext()),
    )
...
def test_dual_system_cross_attn_bad_request_does_not_disable_compile(monkeypatch):
    ...
    actions_next, bridges_next, timestep_next, context_next, context_mask_next = _dual_system_cross_attn_inputs(
        arch, 29
    )
    with torch.no_grad():
