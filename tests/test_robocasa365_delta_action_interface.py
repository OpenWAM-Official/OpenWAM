...
from benchmarks.robocasa365_delta_action import openwam2robocasa365_delta_action_interface as adapter
...
class _Client:
    def __init__(self, action, *, representation="robocasa365_delta_action"):
        self.action = action
...
def test_delta_action15_restores_native_action_without_current_state_or_half_scale():
    action = _action15()
    policy = adapter.OpenWAMRoboCasa365DeltaActionPolicy(_client=_Client(action))
...
def test_representation_handshake_is_independent_from_absolute_robocasa365():
    with pytest.raises(RuntimeError, match="representation mismatch"):
        adapter.OpenWAMRoboCasa365DeltaActionPolicy(
            _client=_Client(_action15(), representation="robocasa365")
        )
