...
def test_reduce_step_metrics_num_processes_one_uses_fast_path():
    ...
    losses = {k: torch.tensor(0.0) for k in ("total", "video", "action", "decoder")}
...
def test_write_debug_loss_row_header_then_append(tmp_path):
    labels = [("action", "loss_action"), ("decoder", "loss_decoder")]
    metrics = {
        "loss_total": 1.0,
        "loss_video": 0.5,
        "loss_action": 0.25,
    ...
    assert lines[0] == ("step,opt_step,epoch,loss,loss_video,loss_action,loss_decoder,grad_norm,lr,steps_per_sec")
...
def test_write_debug_loss_row_columns_follow_labels(tmp_path):
    labels = [("latent_action", "loss_latent_action")]
    metrics = {"loss_total": 1.0, "loss_video": 0.5, "loss_latent_action": 0.3, "grad_norm": 1.0}
    write_debug_loss_row(
        str(tmp_path),
        labels=labels,
        metrics=metrics,
    ...
    assert header == ("step,opt_step,epoch,loss,loss_video,loss_latent_action,grad_norm,lr,steps_per_sec")
