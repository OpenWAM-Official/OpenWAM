...
def test_wrapper_pre_post_attn_dispatch_round_trip():
    ...
    assert torch.allclose(state_mono.x, state_split.x, atol=1e-5, rtol=1e-5)
...
def test_adapter_pre_post_attn_delegate_to_wrapper():
    ...
    assert state.x.shape == (1, 2, 2, 2, 16)
...
# (state.x is 5D for Cosmos25, so the old `vstate.x.shape[1]` would have
...
def test_mot_driver_runs_through_cosmos25_5d_state():
    ...
    ``vstate.x.shape[1]`` which gives ``T`` for Cosmos's 5D state).
    """
    ...
    # state.x is 5D: (1, 2, 2, 2, 16); f*h*w = 2*2*2 = 8.
    ...
    # After the joint loop state.x should still be a finite 5D tensor of the
    # original shape — invariant for the post-attn unflatten.
    assert vstate.x.shape == (1, 2, 2, 2, 16)
